...
def fetch_reuters_ai(url: str = REUTERS_AI_URL) -> str:
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; AIPulseBot/1.0; +https://www.reuters.com/)",
...
def parse_reuters_ai(html: str) -> List[Dict[str, Any]]:
    """
    返回列表：
      - title: 新闻标题
      - url: 新闻链接
    ...
    # Reuters 页面结构可能调整，这里以较宽松的方式匹配 AI 频道下的文章块
...
def update_business_news_with_reuters(
    supabase_url: str,
    service_key: str,
    *,
    record_id: int,
    ...
    payload = {
        "title": title,
        "summary": summary,
        "source_name": "TechCrunch & Reuters",
    }
...
def main() -> None:
    print("开始抓取 Reuters AI 频道...")
    supabase_url, supabase_service_key, llm, llm_model = init_clients()

    try:
        html = fetch_reuters_ai()
    except Exception as e:
        print(f"[错误] 抓取失败：{e}")
        raise

    try:
        articles = parse_reuters_ai(html)
    except Exception as e:
        print(f"[错误] 解析 Reuters 页面失败：{e}")
        raise

    if not articles:
        print("未解析到任何 Reuters AI 文章，结束。")
        return
    ...
    print(f"[Reuters Sync] 发现 {len(articles)} 条候选深度报道，开始处理...")

    for idx, art in enumerate(articles, start=1):
        title = art["title"]
        url = art["url"]
        published = _parse_published_to_iso(art.get("published", ""))
        ...
        if conflict is not None:
            ...
            if record_id is None:
                ...
                try:
                    update_business_news_with_reuters(
                        supabase_url,
    ...
    print(f"[Reuters Sync] 发现 {len(articles)} 条深度报道，升级了 {upgraded} 条快讯记录，新增 {inserted} 条。")
